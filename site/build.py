...
def copy_static():
    ...
    # Product images — copy the whole folder (~1,780 files, ~30 MB)
    dst_prod = os.path.join(DIST, "assets", "product")
    src_prod = os.path.join(ASSETS_SRC, "product")
    if os.path.isdir(src_prod):
        shutil.copytree(src_prod, dst_prod, dirs_exist_ok=True)
    ...
    for t in d.get("tabs", []):
        ...
        if kind == "cards":
            ...
            for c in (t.get("cards") or []):
                ...
                if not (title or cap or c.get("image")):
                    continue
                cards.append({"title": title, "caption": cap, "image": c.get("image")})
            if not cards:
                continue
            out.append({"key": key, "label": label, "kind": "cards", "cards": cards})
            ...
            for b in (t.get("blocks") or []):
                ...
                if not (dims or mat or desk or items or b.get("image")) and not model:
                    continue
                if not (dims or mat or desk or items) and not b.get("image"):
                    continue
                blocks.append({"model_no": model, "dimensions": dims, "material": mat,
                               "desktop": desk, "items_included": items, "image": b.get("image")})
            if not blocks:
                continue
            out.append({"key": key, "label": label, "kind": "spec_blocks", "blocks": blocks})
        elif kind == "generic":
            blocks = []
            for b in (t.get("blocks") or []):
                ...
                if not (title or body or b.get("image") or table):
                    continue
                blocks.append({"title": title, "body": body, "image": b.get("image"), "table": table})
            if not blocks:
                continue
...
def build_family(cat_slug, cat_meta, family_info, variants):
    ...
    html = env.get_template("family.html").render(
        page_title=f"{fam_name} — {cat_meta['name']} | Primaxs"[:65],
        meta_description=f"{fam_name} in Malaysia — {len(variants)} variants with side-by-side specs. Exclusive Tanko distributor."[:158],
        canonical=f"https://primaxs.com.my/{cat_slug}/{family_info['family_slug']}/",
...
def build_variant(cat_slug, cat_meta, family_info, variant, all_family_variants):
    ...
    # Title / meta with variant-specific content
    ...
    # Keep the whole "| Primaxs Malaysia" suffix; only shorten title_core if needed.
    suffix = " | Primaxs Malaysia"
    max_len = 70
...
def build_products_index(products):
    ...
    html = env.get_template("products_index.html").render(
        ...
        categories=cat_cards, base_url=BASE_URL, year=YEAR, json_ld=org_json_ld(),
    )
...
def build_guides():
    ...
    html = env.get_template("guides_index.html").render(
        ...
        guides=cards, base_url=BASE_URL, year=YEAR, json_ld=org_json_ld(),
    )
    ...
    for g in GUIDES:
        ...
        combined_ld = "[" + article_ld + ("," + faq_ld if faq_ld else "") + "]"
...
def build_subcollections(families, prods_by_family):
    ...
    for (cat_slug, sub_slug), data in groups.items():
        ...
        html = env.get_template("category.html").render(
            ...
            canonical=f"https://primaxs.com.my/{cat_slug}/{sub_slug}/",
...
def main():
    ...
    write(os.path.join(DIST, "about", "index.html"),
          env.get_template("about.html").render(
              ...
              meta_description="Primaxs Marketing (M) Sdn Bhd — Malaysia's exclusive Tanko industrial storage distributor since 2006. Selangor-based, nationwide delivery, local warranty support.",
